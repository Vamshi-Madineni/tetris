...
def process_tetris_actions(action_sequence: list[str]):
    ...
    """
    playfield = []  # Create the initial line of the playfield
    playfield.append(0)
...
def add_shape_to_playfield(action, playfield, column_tops):
    ...
    Args:
        ...
        playfield (list[int]): The current playfield grid.
...
def remove_completed_rows(playfield, column_tops):
    ...
    Args:
        playfield (list[int]): The current playfield.
        column_tops (list[int]): List tracking the highest occupied cell in each column.
    """
    rows_to_remove = []
    new_playfield = []
    ...
    for col_index in range(len(column_tops)):
        ...
        # Count how many rows in rows_to_remove are less than original_top
        removed_count = bisect_left(rows_to_remove, original_top)
